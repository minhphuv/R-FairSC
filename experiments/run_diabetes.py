"""Run R-FairSC on the Diabetes dataset."""
...
def summarize_results(results: list[dict]) -> pd.DataFrame:
    ...
    for k in sorted({int(result["k"]) for result in results}):
        ...
        x_runtimes = np.asarray([run["X_runtime"] for run in runs], dtype=float)
        ...
        rows.append(
            {
                ...
                "X_runtime_mean": np.nanmean(x_runtimes),
                "X_runtime_std": _sample_std(x_runtimes),
    ...
    for k in args.clusters:
        for run_index in range(args.num_runs):
            ...
            results.append(
                {
                    ...
                    # Preserve the original Diabetes CSV field name.
                    "X_runtime": solution.h_runtime,
            ...
            print(
                ...
                f"X_time={solution.h_runtime:.3f}s "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run R-FairSC on Diabetes.")
    parser.add_argument(
        "--edges",
        default="data/diabetes/edges.csv",
        help="Path to edges.csv.",
    )
    parser.add_argument(
        "--colors",
        default="data/diabetes/gender.csv",
        help="Path to gender.csv.",
    )
    parser.add_argument(
        "--color-column",
        type=int,
    ...
    parser.add_argument(
        "--output",
        default="results/r_fairsc_gpu_diabetes_time_fair_x.csv",
